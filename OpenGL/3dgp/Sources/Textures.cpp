...
short Textures::cnt_gl_texture_id_count = 0;
...
bool Textures::AddNew2DTexture(std::string path) {//if true -> do nothing else -> return false in init
	...
	short l = cnt_gl_texture_id_count;
	glActiveTexture(GL_TEXTURE0 + l);
	cnt_gl_texture_id_count++;
	idTex.push_back(GLuint());
	glGenTextures(1, &idTex[l]);
	glBindTexture(GL_TEXTURE_2D, idTex[l]);
